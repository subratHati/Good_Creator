                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)' }}>
                      {profile?.instagram?.isConnected
                        ? 'Connected · tap sync to refresh'
                        : profile?.instagram?.handle
                          ? `@${profile.instagram.handle} · self-reported`
                          : 'Not connected'}
                    </div>
                  </div>
                </div>
          ...
          <div className="hidden md:flex gap-6 items-start px-4 md:px-0">
            ...
            <div style={{ width: '260px', flexShrink: 0, position: 'sticky', top: '90px' }}>
              <div style={{ backgroundColor: '#101828', borderRadius: '24px', padding: '24px 20px', boxShadow: '0 6px 0 0 #0a1020', textAlign: 'center' }}>
                ...
                <div style={{ background: profile?.instagram?.isConnected ? 'linear-gradient(90deg,#833AB4,#E1306C)' : profile?.instagram?.handle ? '#F3F4F6' : '#F3F4F6', borderRadius: '10px', padding: '8px', marginBottom: '16px' }}>
                  <span style={{ fontSize: '11px', fontWeight: 900, color: profile?.instagram?.isConnected ? 'white' : profile?.instagram?.handle ? '#374151' : '#9CA3AF' }}>
                    {profile?.instagram?.isConnected
                      ? `📸 @${profile.instagram.handle}`
                      : profile?.instagram?.handle
                        ? `📸 @${profile.instagram.handle} (self-reported)`
